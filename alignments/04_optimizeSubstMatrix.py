...
# names=[leaf.name for leaf in leafs]
# #print(names)
# ratioActivesTotal=float(len(actives))/len(names)
# ratioInactivesTotal=float(len(inactives))/len(names)
# #print(ratioActivesTotal,ratioInactivesTotal)

# clade=tree.root
# efDict=getEnrichmentForClade(clade)
# from functools import reduce 
# #efDictFinal=[reduce(lambda x,y: {**x,**y},d.values()) for d in efDict]

# minN=80
# print(len([key for key in efDict.keys() if (efDict[key][2]>minN and efDict[key][0]>1.0)]))
# print(np.mean([efDict[key][0] for key in efDict.keys() if (efDict[key][2]>minN and efDict[key][0]>1.0)]))
# print(np.mean([efDict[key][1] for key in efDict.keys() if (efDict[key][2]>minN and efDict[key][1]>1.0)]))
...
# nActivesRatio=nActivesInClade/float(len(names))
# nInactivesRatio=nInactivesInClade/float(len(names))

# efActives=nActivesRatio/ratioActivesTotal
# efInactives=nInactivesRatio/ratioInactivesTotal
# print(efActives,efInactives)
